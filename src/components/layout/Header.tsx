...
export function Header() {
 ...
 return (
 <header
 className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${useSolidBackground ? 'bg-white/95 shadow-sm backdrop-blur-md ' : 'bg-transparent'
 }`}
 >
 ...
 <span className={`text-xl font-bold tracking-tight ${!isTranslucentHeader ? 'text-navy-900 ' : 'text-white'}`}>
 Kaisyoku
 </span>
 </Link>
 ...
 >
 <span className={`text-sm font-bold transition-colors ${!isTranslucentHeader ? 'text-navy-700 group-hover:text-main ' : 'text-white/90 group-hover:text-white'}`}>
 {item.label}
 </span>
 ...
 <button
 className={`md:hidden p-2 focus:outline-none ${!isTranslucentHeader ? 'text-navy-900 ' : 'text-white'}`}
